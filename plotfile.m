...
function plotfile(nlay, rc, efilter, vel, prob)

nameplot=['Fig' prob vel num2str(nlay) rc num2str(round(efilter*1000))]; %Fig nlay constraints filter
load([nameplot '.mat'])
...
%plot each layer
% for lay=1:nlay
%     plotfun(COORD,dvf((lay-1)*nd+(1:nd)),dve((lay-1)*ne+(1:ne)),data,nameplot,lay,lay,vel,nmax);
%     hold off
%     dva=dva+dve((lay-1)*ne+(1:ne))/nlay;
% end
% 
% for lay=1:nlay
%     plotfun(COORD,dvf((lay-1)*nd+(1:nd)),dva,data,nameplot,nlay+1,lay,vel,nmax);
% end
% hold off
